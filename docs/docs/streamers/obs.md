...
```text
https://qrtuber.com/app/generator/?overlay=1&size=360&ecc=M&rate=5&ch1=180:sine
```
...
The overlay URL hides generator controls and renders only the QR output. You can use **Copy OBS URL** whenever you change the generator's QR size, error correction, update rate, or channel pattern setup.
...
```text
https://qrtuber.com/app/device/?overlay=1&connect=1&size=360&device=qrtuber-lovense
```

Device overlay parameters include `server=ws://127.0.0.1:54817`, `device=...`, and `id=...`.
Device mode also has **Copy OBS URL** in its QR panel, using the current server, identifier, device address, QR size, and error correction settings.

## Testing

For local testing, open the generator controls and the web viewer at the same time:

1. Open a configured generator URL, such as https://qrtuber.com/app/generator/?overlay=1&size=360&rate=5&ch1=180:sine.
